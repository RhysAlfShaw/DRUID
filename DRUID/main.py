...
class sf:
    ...
    def __init__(
        ...
        if mode == "Radio":
            pass
            # self.image = np.pad(
            #     self.image, ((1, 1), (1, 1)), mode="constant", constant_values=0
            # )
        ...
        if self.cutup:
            ...
            if self.pb_path is not None:
                self.pb_cutouts, self.pb_coords = utils.cut_image(
                    cutup_size, self.pb_image
                )
    ...
    def phsf(self, lifetime_limit: float = 0, lifetime_limit_fraction: float = 2):
        ...
        if self.cutup == True:
            ...
            for i, cutout in tqdm(
                ...
                catalogue = homology.compute_ph_components(
                    ...
                    mean_bg=self.mean_bg,
                    IDoffset=IDoffset,
                    box_size=self.box_size,
                    detection_threshold=self.sigma,
                    Cutout_X_offset=self.coords[i][1],
                    Cutout_Y_offset=self.coords[i][0],
                )
                if len(catalogue) == 0:
                    continue
                ...
                catalogue["distance_from_center"] = (
                    (catalogue["x1"] - cutout.shape[0] / 2) ** 2
                    + (catalogue["y1"] - cutout.shape[1] / 2) ** 2
                ) ** 0.5
            ...
            if self.remove_edge:
                ...
                self.catalogue = self.catalogue.sort_values(
                    by=["distance_from_center"], ascending=True
                )
                self.catalogue = self.catalogue.drop_duplicates(
                    subset=["x1", "y1", "Birth"], keep="first"
                )
            else:
        ...
        t0 = time.time()
        for i in tqdm(
            range(0, len(self.catalogue)),
            total=len(self.catalogue),
            desc="Calculating enclosed_i",
            disable=not self.output,
        ):
            ...
            if self.GPU == True:
                ...
                enclosed_i = homology.make_point_enclosure_assoc_GPU(
                    0, x1, y1, Birth, Death, cat, img_gpu
                )
                enclosed_i_list.append(enclosed_i)
            else:
                # print(self.catalogue)
                enclosed_i = homology.make_point_enclosure_assoc_CPU(
                    0, x1, y1, Birth, Death, cat, img
                )
                enclosed_i_list.append(enclosed_i)
        ...
        # print(self.catalogue)
        # print("Enclosed_i calculated! TESTESTEST")
        ...
        self.catalogue = homology.correct_first_destruction(
            self.catalogue, output=not self.output
        )
        ...
        t0_parent_tag = time.time()
        self.catalogue = homology.parent_tag_func_optimized(self.catalogue)
        t1_parent_tag = time.time()
            ...
            if self.cutup:
                ...
                if bg_map is not None:
                    ...
                    for i, cutout in enumerate(self.cutouts):
                        local_bg_map, mean_bg = background.radio_background_map(
                            cutout, box_size
                        )
                        analysis_threshold_list.append(
                            local_bg_map * self.analysis_threshold
                        )
                    ...
                    for cutout in self.cutouts:
                        ...
                        analysis_threshold_list.append(
                            local_bg * self.analysis_threshold
                        )
                ...
                if bg_map is not None:
                    local_bg_o, mean_bg = background.radio_background_map(
                        self.image, box_size
                    )
    ...
    def source_characterising(self, use_gpu: bool = False):
        ...
        if self.mode == "optical":
            ...
            def Flux_err(EFFRON, EFFGAIN, EXPTIME, Area, sky, Flux):
                return np.sqrt(
                    RONoise(EFFRON, EFFGAIN, EXPTIME, Area) ** 2
                    + SkyNoise(sky)
                    + SourceNoise(Flux)
                )
            ...
            def NOISE(row, local_ng):
                return np.sum(
                    np.random.normal(row["mean_bg"], local_ng, int(row["Area"]))
                )
            ...
            self.catalogue["Flux_total_new"] = (
                self.catalogue["Flux_total"] * EFFGAIN * EXPTIME
                - self.catalogue["Noise"] * EFFGAIN * EXPTIME
                - RONoise(EFFRON, EFFGAIN, EXPTIME, self.catalogue["Area"])
            )
            # print('Flux_total_new: ',self.catalogue['Flux_total_new'])
            self.catalogue["Flux_total_err"] = Flux_err(
                EFFRON,
                EFFGAIN,
                EXPTIME,
                self.catalogue["Area"],
            ...
            self.catalogue["SNR"] = (
                self.catalogue["Flux_total_new"]
                / self.catalogue["Flux_total_err"]
                / self.catalogue["Area"]
            )
            # print('SNR: ',self.catalogue['SNR'])
            self.catalogue["MAG_err"] = (
                1 / self.catalogue["SNR"]
            )  # use the approximate error for the magnitude.
            self.catalogue["Flux_total_new"] = self.catalogue["Flux_total_new"] / (
                EFFGAIN * EXPTIME
            )
    ...
    def create_polygons(self, use_gpu=False):
        ...
        self.catalogue = source.create_polygons(
            use_gpu=use_gpu,
            catalogue=self.catalogue,
            cutout=self.image,
            output=self.output,
    ...
    def create_polygons_fast(self):
        ...
        for index, row in tqdm(
            ...
            contour = utils._get_polygons_in_bbox(
                row.bbox2 - 2,
                row.bbox4 + 2,
                row.bbox1 - 2,
                row.bbox3 + 2,
    ...
    def open_catalogue(self, file_path, filetype=None):
        ...
        for i in range(len(self.catalogue)):
            self.catalogue["contour"][i] = np.array(
                self.catalogue["contour"][i]
            ).reshape(-1, 2)
        ...
        with open(filename, "w") as f:
            ...
            for polygon in self.polygons:
                ...
                for i, point in enumerate(polygon):
                    f.write(
                        "{:.2f},{:.2f}".format(point[1], point[0])
                    )  # note this transformation as the index in some CARTA inmages start at -1.
                    if i < len(polygon) - 1:
                        f.write(",")
                f.write(")\n")
