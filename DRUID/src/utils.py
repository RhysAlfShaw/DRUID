...
def remove_duplicates(catalogue):
    ...
    catalogue = catalogue.drop_duplicates(
        subset=["x1", "y1", "Birth", "Class"], keep="first"
    )
    # sort by ID
    # sort by ID
...
def _get_polygons_gpu(x1: int, y1: int, birth: float, death: float, image):
    """
...
def _get_polygons_in_bbox(
    Xmin, Xmax, Ymin, Ymax, x1, y1, birth, death, mask, X0, Y0, pad=1
):
