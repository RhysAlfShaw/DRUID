...
def measure_source_properties(
    ...
    for i, source in tqdm(
        enumerate(Birth),
        total=len(Birth),
        desc="Calculating Source Properties..",
        disable=not output,
    ):

        if use_gpu == True:

            cropped_image_gpu = image_gpu[
        ...
        contour = utils._get_polygons_in_bbox(
            xmin, xmax, ymin, ymax, x1[i], y1[i], Birth[i], Death[i], red_mask, 0, 0
        )

        # TEST #
        # from matplotlib.pylab import plt

        # plt.imshow(red_image, cmap="gray", origin="lower")
        # plt.imshow(red_mask, alpha=0.5, origin="lower")
        # # plot the countour of the red_mask
        # plt.plot(contour[:, 1] - xmin, contour[:, 0] - ymin, color="red")
        # plt.show()
        ...
        # print('Class',Class[i])
        SNR = 0  # default value gets overwritten if calculated
        if mode == "Radio" or mode == "optical":
            ...
            if shape[0] < 100:
                ...
                red_image = np.pad(
                    red_image, ((pad, pad), (0, 0)), mode="constant", constant_values=0
                )
                red_background_mask = np.pad(
                    red_background_mask,
                    ((pad, pad), (0, 0)),
                    mode="constant",
                    constant_values=0,
                ...
                red_mask = np.pad(
                    red_mask, ((pad, pad), (0, 0)), mode="constant", constant_values=0
                )
                shape = red_image.shape
                y = y_peak_loc + pad
            ...
            if shape[1] < 100:
                ...
                red_image = np.pad(
                    red_image, ((0, 0), (pad, pad)), mode="constant", constant_values=0
                )
                red_background_mask = np.pad(
                    red_background_mask,
                    ((0, 0), (pad, pad)),
                ...
                red_mask = np.pad(
                    red_mask, ((0, 0), (pad, pad)), mode="constant", constant_values=0
                )
                shape = red_image.shape
                x = x_peak_loc + pad
            ...
            if mode == "optical":
                ...
                Model_Beam = utils.model_beam_func(
                    source_props["max_intensity"], shape, x, y, MAJ, MIN, BPA
                )
                # plt.figure(figsize=(10,10))
                ...
                Model_Beam = utils.model_beam_func(
                    source_props["max_intensity"], shape, x, y, BMAJ / 2, BMIN / 2, BPA
                )

            if mode == "Radio":
                Flux_total = (
                    np.nansum(red_mask * red_image - red_background_mask) / Beam
                )
            else:
            ...
            Flux_peak = (
                np.nanmax(red_mask * red_image)
                - red_background_mask[y_peak_loc, x_peak_loc]
            )
            ...
            if mode == "Radio":
                ...
                Flux_total_err = rms * np.sqrt(Area / Beam_area)
                SNR = Flux_total / Flux_total_err
